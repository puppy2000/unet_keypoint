...
def calculate_prediction_metrics(result_dict: dict):
    ...
    for file_path, landmark_dict in result_dict.items():
        ...
        each_landmark_error = np.sqrt(np.sum(np.square(landmarks - predict_landmarks), axis=1)) 
        each_landmark_error = each_landmark_error * scale
...
def visualize_prediction_landmarks(result_dict: dict, save_image_dir: str):
    ...
    for file_path, landmark_dict in result_dict.items():
        ...
        for i in range(np.shape(landmarks)[0]):
            ...
            rr, cc = sk_draw.disk(center=(int(predict_landmark[0]), int(predict_landmark[1])), radius=radius, shape=image_shape)
            image[rr, cc, :] = [0, 255, 0]
            # the line between gt landmark and prediction landmark
            # line_width = 5
            # rr, cc, value = sk_draw.line_aa(int(landmark[1]), int(landmark[0]), int(predict_landmark[0]), int(predict_landmark[1]))
        ...
        sk_io.imsave(os.path.join(save_image_dir, str(count)+'.jpg'), image)
