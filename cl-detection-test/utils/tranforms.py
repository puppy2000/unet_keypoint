...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        image = sk_transform.resize(image, (new_h, new_w), mode='constant', preserve_range=True)
        image = image / 255
        landmarks = landmarks * [scale_w, scale_h]
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for i in range(n_landmarks):
            center_x,center_y = int(landmarks[i, 0]+0.5), int(landmarks[i, 1]+0.5)
            heatmap[i, :, :] = generate_2d_gaussian_heatmap(heatmap[i, :, :], (center_x,center_y), sigma=8, radius=20)
