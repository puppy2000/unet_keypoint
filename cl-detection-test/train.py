...
def main(config):
    ...
    train_transform = transforms.Compose([Rescale(output_size=(config.image_height, config.image_width)),
                                        #   RandomHorizontalFlip(p=config.flip_augmentation_prob),
                                          ToTensor()])
    train_dataset = CephXrayDataset(txt_file_path=config.train_txt_path, transform=train_transform)
    valid_transform = transforms.Compose([Rescale(output_size=(config.image_height, config.image_width)),
                                          ToTensor()])
    valid_dataset = CephXrayDataset(txt_file_path=config.valid_txt_path, transform=valid_transform)
    ...
    for epoch in range(config.train_max_epoch):
        ...
        for d in tqdm.tqdm(train_loader):
            ...
            writer.add_scalar('train_loss',loss,iter_num_train)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            model.eval()
            print("Validating....")
            for d in tqdm.tqdm(valid_loader):
                image, heatmap = d['image'], d['heatmap']
                image, heatmap = image.float().to(device), heatmap.float().to(device)
                output = model(image)
                loss = loss_fn(output, heatmap)
                writer.add_scalar('val_loss',loss,iter_num_val)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_txt_path', type=str, default='/data/detection/cl-det-fix/train.txt')
    parser.add_argument('--valid_txt_path', type=str, default='/data/detection/cl-det-fix/val.txt')
    ...
    # result & save
    parser.add_argument('--save_model_dir', type=str, default='/home/zt/cl-detection-test/model/')
